...
Tudo roda **gratuitamente** no Cloudflare (Workers + D1 + R2).
...
└── backend/    Hono no Cloudflare Workers (API REST + D1 + R2)
```

- **Frontend:** React 18, React Router, Framer Motion, CSS Modules. Tema mariano
  (branco, dourado e azul de Fátima).
- **Backend:** Hono. Banco **D1** (SQLite) para conteúdo, **R2** para as fotos.
...
```bash
# 1) Backend (Worker + D1 + R2 simulados localmente)
cd backend
npm install
